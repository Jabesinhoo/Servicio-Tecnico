...
if (-not $hasAbsoluteRoute -and -not $hasNestedRoute) {
    ...
    # C. Fallback robusto: soporta rutas JSX escritas en varias lineas.
    $routeEndIndex = -1
    ...
    if ($routeIndex -lt 0) {
        for ($i = 0; $i -lt $appLines.Count; $i++) {
            ...
            if ($appLines[$i] -match 'path\s*=\s*["''](\/dashboard\/)?servicios["'']') {
                ...
                # Buscar hacia arriba el <Route que pertenece a este path.
                $start = $i

                while (
                    $start -ge 0 -and
                    $appLines[$start] -notmatch '<Route\b'
                ) {
                    $start--
                }

                if ($start -lt 0 -or ($i - $start) -gt 10) {
                    continue
                }

                # Buscar hacia abajo el cierre /> del mismo Route.
                $end = $i

                while (
                    $end -lt $appLines.Count -and
                    $appLines[$end] -notmatch '\/>\s*$'
                ) {
                    $end++
                }

                if ($end -ge $appLines.Count -or ($end - $i) -gt 10) {
                    continue
                }

                $routeBlock = ($appLines[$start..$end] -join "`n")

                if ($routeBlock -match '<Servicios\b') {
    ...
    # Si una deteccion anterior encontro el inicio de la ruta,
    # calcular igualmente donde termina realmente.
    if ($routeEndIndex -lt 0) {

        $routeEndIndex = $routeIndex
        $maxRouteEnd = [Math]::Min(
            $appLines.Count - 1,
            $routeIndex + 10
        )

        while (
            $routeEndIndex -le $maxRouteEnd -and
            $appLines[$routeEndIndex] -notmatch '\/>\s*$'
        ) {
            $routeEndIndex++
        }

        if ($routeEndIndex -gt $maxRouteEnd) {
            throw "Localice Servicios, pero no pude determinar donde termina su bloque <Route />."
        }
    }
    ...
    # MUY IMPORTANTE:
    # insertar DESPUES del bloque completo de Servicios.
    $appLines.Insert(
        $routeEndIndex + 1,
        $newRoute
    )
