...
exports.getEventos = async (req, res) => {
  try {
    ...
    let query = `
      SELECT 
        so.id, so.codigo_os, so.estado, so.descripcion_inicial,
        so.fecha_agendada, so.hora_inicio_agendada, so.duracion_estimada,
        c.nombre_razon_social as cliente_nombre,
        u.id as tecnico_id, u.nombre1 as tecnico_nombre, u.apellidos as tecnico_apellidos,
        CASE 
      ...
      WHERE so.fecha_agendada IS NOT NULL
        AND so.estado NOT IN ('cerrada', 'cancelada')
        AND ($1::date IS NULL OR so.fecha_agendada >= $1)
        AND ($2::date IS NULL OR so.fecha_agendada <= $2)
    `;
    ...
    query += ` ORDER BY so.fecha_agendada ASC, so.hora_inicio_agendada ASC`;
    
    const result = await pool.query(query, params);
    
    // Formatear eventos para FullCalendar o similar
    const eventos = result.rows.map(evento => ({
      id: evento.id,
      title: `${evento.codigo_os} - ${evento.cliente_nombre_completo || 'Cliente'}`,
      start: `${evento.fecha_agendada.toISOString().split('T')[0]}T${evento.hora_inicio_agendada || '08:00'}`,
      end: calcularHoraFin(evento.fecha_agendada, evento.hora_inicio_agendada, evento.duracion_estimada || 60),
      backgroundColor: getColorPorEstado(evento.estado),
      extendedProps: {
        estado: evento.estado,
        codigo_os: evento.codigo_os,
        cliente: evento.cliente_nombre_completo,
        descripcion: evento.descripcion_inicial,
        tecnico_id: evento.tecnico_id,
        tecnico_nombre: `${evento.tecnico_nombre || ''} ${evento.tecnico_apellidos || ''}`,
        duracion: evento.duracion_estimada
      }
    }));
...
function calcularHoraFin(fecha, horaInicio, duracionMinutos) {
  ...
  fechaObj.setHours(parseInt(horas), parseInt(minutos) + duracionMinutos);
